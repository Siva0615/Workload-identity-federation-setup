...
**Via `gcloud` (file-sourced credentials — recommended for local dev):**

This approach uses a **file** to hold your Google identity token. You refresh the file before each session with a single `gcloud` command.

**Step A — Create the credential config (run once):**
...
**Step B — Refresh the identity token before each session:**

```bash
# Linux / macOS
gcloud auth print-identity-token \
    --audiences="https://iam.googleapis.com/projects/YOUR_PROJECT_NUMBER/locations/global/workloadIdentityPools/YOUR_POOL_ID/providers/YOUR_PROVIDER_ID" \
    > /tmp/oidc-token.txt
```

```powershell
# Windows (PowerShell) — uses %TEMP%\oidc-token.txt; update --credential-source-file path to match
gcloud auth print-identity-token `
    --audiences="https://iam.googleapis.com/projects/YOUR_PROJECT_NUMBER/locations/global/workloadIdentityPools/YOUR_POOL_ID/providers/YOUR_PROVIDER_ID" |
    Set-Content -Path "$env:TEMP\oidc-token.txt" -NoNewline
```

> **Why this approach?**  
> The `--credential-source-file` flag tells `google-auth` to read the raw identity token from a local file. When the token expires (~1 hour), just re-run the Step B command. This is simpler than executable-sourced credentials, which require a wrapper script that outputs a specific JSON format.

> **Important:** Add `credential-config.json` and the token file to `.gitignore`:
> ```bash
> echo "credential-config.json" >> .gitignore
> echo "oidc-token.txt" >> .gitignore
> ```
...
### 5. Refresh the identity token (required before each session)

The credential config reads a token from a local file. That file must exist and be fresh (tokens expire after ~1 hour). Run this **once per session** before `python main.py`:
...
> **Tip:** The `--audiences` value must match the provider audience used when you ran `create-cred-config` in Step 6.
...
The file generated by Step 6 looks like this (file-sourced credentials):
...
`google-auth` reads the raw identity token from the file path listed in `credential_source.file`, then automatically exchanges it for a short-lived GCP access token via STS.
...
| `invalid_grant` or `Token has been expired` | `gcloud` token expired | Re-run the Step B token refresh command from Step 6 |
...
| `unrecognized arguments: --credential-source-command` | Invalid flag — only exists in older docs | Use `--credential-source-file` (see Step 6) |
| `unrecognized arguments: --executable-output-format` | This flag does not exist in any gcloud version | Remove it; use `--credential-source-file` approach instead |
